...
export class CreateShoppingListHandler implements ICommandHandler<CreateShoppingList> {
    ...
    async execute(command: CreateShoppingList) {
        let shoppingList = await this.shoppingListRepository.findById(command.shoppingListId);
